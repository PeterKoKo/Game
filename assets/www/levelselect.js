...
levelSelect = {
  	create: function(){
		...
		leftArrow = game.add.button(20,window.innerHeight / 2,"level_arrows",this.arrowClicked,this);
		...
		rightArrow = game.add.button(window.innerWidth -30 ,window.innerHeight / 2,"level_arrows",this.arrowClicked,this);
		...
		for(var l = 0; l < pages; l++){
			...
			var offsetY = (game.height-levelHeight)/2; //20;
	...
	thumbClicked:function(button){
		...
		if(button.frame < 4){
			...
			game.state.start("level" + game.global.level); //game.global.level);
		}
